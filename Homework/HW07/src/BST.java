...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		TreeIterator submax = (TreeIterator) subtree.findMax();
		TreeIterator min = (TreeIterator) findMin(root);
		if(submax.currentNode.data<min.currentNode.data) {
			min.currentNode.left=subtree.root;
			subtree.root.parent=min.currentNode;
			size+=subtree.size;
		}
		else {
			return;
